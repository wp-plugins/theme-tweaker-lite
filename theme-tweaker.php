...
if (!class_exists("ThemeTweaker")) {
  ...
  class ThemeTweaker {
    ...
    function printAdminPage() {
      ...
      if ($colors0 == $TTOptions[$mOldKey]) {
        $colors1 = $TTOptions[$mNewKey];
      }
      else {
        $colors1 = $colors0;
      }
      ...
      if (!$TTOptions['kill_rating']) {
        renderRating($myPlugins[$plgName], $plgDir);
      }
      if (!$TTOptions['kill_invites']) {
...
if (class_exists("ThemeTweaker")) {
  ...
  if (isset($thmTwk)) {
    ...
    add_action('activate_' . basename(dirname(__FILE__)) . '/' . basename(__FILE__), array($thmTwk, 'init'));
